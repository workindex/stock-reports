...
- `docs/` — 발행 보고서 (gen.py 자동 생성, 직접 편집 금지). `홈·관찰 종목·분석 스냅샷·알림` 4섹션
  - `docs/alerts/` — **monitor.py가 직접 push** (gen.py는 인덱스만 만들고 삭제하지 않음)
- `gen.py` — 메인 저장소 워치리스트/스냅샷 → 사이트 docs 변환 + 섹션 인덱스 생성
- `mkdocs.yml` — MkDocs Material 설정 (nav 4메뉴)
- `.github/workflows/deploy.yml` — push 시 GitHub Pages 자동 배포

> 판정 어휘·메뉴 구조의 단일 기준: 메인 저장소 [`docs/verdict-taxonomy.md`](../docs/verdict-taxonomy.md)
